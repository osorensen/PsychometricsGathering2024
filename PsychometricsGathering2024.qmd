...
- Regularization parameters for smooth terms are top-level random effect variances.
...
## Limits to deterministic algorithms?

Intense longitudinal data, dynamic SEM [@asparouhovDynamicStructuralEquation2018;@mcneishPrimerTwolevelDynamic2020]:

- Hamiltonian Monte Carlo (`Stan`) seems to be the most practical computational option. 
- Work in progress on implementing splines to captures trends, cycles, and learning effects in DSEMs for EdTech (forthcoming: Sørensen and McCormick; McCormick and Sørensen).
